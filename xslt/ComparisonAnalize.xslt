    ...
    <xsl:for-each select="$Control_Field">
      ...
      <xsl:choose>
        <xsl:when test="IsExist = 'yes'">
          ...
          <xsl:choose>
            <xsl:when test="ForeignKey = '1'">
              <xsl:if test="ForeignKeyExist != 'yes'">
                  <sql>
                    <xsl:value-of select="concat('ALTER TABLE ', $TableName, ' ADD CONSTRAINT ', 'fk_', $TableName, '_', NameInTable, '_references ', 
                        'FOREIGN KEY(', NameInTable, ') REFERENCES ', References, '(uid) ON UPDATE NO ACTION ON DELETE NO ACTION NOT VALID;')"/>
                  </sql>
              </xsl:if>
            </xsl:when>
            <xsl:otherwise> 
              <xsl:if test="ForeignKeyExist = 'yes'">
                  <sql>
                    <xsl:value-of select="concat('ALTER TABLE ', $TableName, ' DROP CONSTRAINT IF EXISTS ', 'fk_', $TableName, '_', NameInTable, '_references')"/>
                  </sql>
              </xsl:if>
            </xsl:otherwise>
          </xsl:choose>
    ...
    <root>
      ...
      <xsl:for-each select="root/Control_Table">
        ...
        <xsl:for-each select="Control_TabularParts">
          ...
          <xsl:variable name="OwnerTableName" select="OwnerTableName" />
